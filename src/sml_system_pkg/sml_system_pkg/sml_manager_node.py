...
class SmlManagerNode(Node):
    ...
    def _on_amr_assemble_arm_result(self, future, step, retry):
        try:
            ...
            # A lost response does not prove that the physical command was not
            # executed. Re-sending ASSEMBLE could consume the same inputs twice.
            self._fail_amr_assemble_step(
        ...
        if not response.success:
            ...
            normalized_message = ' '.join(
                message.lower().replace('_', ' ').split())
            # "busy" is returned before any robot motion starts, so it is the
            # only response that is safe to retry as a whole command.
            if retry < 1 and normalized_message == 'busy':
                nav_target = self._get_amr_assemble_nav_target(step)
                self.get_logger().warn(
    ...
    def _execute_arm(self, step, retry=0, object_offset=0):
        MAX_RETRY = 1
        ...
        all_arm_slide_ids = self._set_arm_request_slide_ids(
            req, step, req.action)
        arm_slide_ids = all_arm_slide_ids[object_offset:]
        if object_offset and hasattr(req, 'slide_ids'):
            req.slide_ids = list(arm_slide_ids)

        logical_slide_ids = list(
            getattr(step, 'slide_ids', []))[object_offset:]

        self.get_logger().info(
            f'[ARM] step {step.step_id} → '
            f'{req.action} {list(req.object_ids)} | location={req.location} | '
            f'logical_slide_ids={logical_slide_ids} | '
            f'arm_slide_ids={arm_slide_ids}')

        future = self.arm_client.call_async(req)
        future.add_done_callback(
            lambda f, s=step, r=retry, o=object_offset:
            self._on_arm_result(f, s, r, o))

    def _on_arm_result(self, future, step, retry, object_offset=0):
        try:
            response = future.result()
        except Exception as e:
            self.get_logger().error(
                f'[ARM] step {step.step_id} 예외: {e}')
            # The service may have completed physical work even when its
            # response is lost, so blindly re-sending the command is unsafe.
            self._set_amr_idle(step)
            return
        ...
        if not response.success:
            ...
            normalized_message = ' '.join(
                response.message.lower().replace('_', ' ').split())

            is_load_vision_miss = (
                step.action == Step.LOAD
                and 'object not found' in normalized_message
            )
            if retry < 1 and is_load_vision_miss:
                # The AMR returns one result per attempted object and stops at
                # the first failure. Every result before the last one is a
                # completed load, so retry from the failed object's offset.
                attempted = len(getattr(response, 'object_ids', []))
                completed_prefix = max(0, attempted - 1)
                retry_offset = object_offset + completed_prefix
                remaining = list(step.object_ids)[retry_offset:]

                if remaining:
                    self.get_logger().warn(
                        f'[ARM] step {step.step_id} 비전 실패 물체부터 '
                        f'재시도 ({retry+1}/1) | objects={remaining}')
                    self._execute_arm(
                        step, retry + 1, object_offset=retry_offset)
                    return

            # "busy" is rejected before execution and is therefore safe to
            # retry with the same object range.
            if retry < 1 and normalized_message == 'busy':
                self.get_logger().warn(
                    f'[ARM] step {step.step_id} 재시도 ({retry+1}/1)')
                self._execute_arm(
                    step, retry + 1, object_offset=object_offset)
            else:
                self.get_logger().error(
