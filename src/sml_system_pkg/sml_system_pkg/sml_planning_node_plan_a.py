"""ROS2 planning node wrapper for Plan A.
...
from .planning_plan_a.arena_parser import load_station_coord_json
from .planning_plan_a.planner_config import (
    AMR_SPEED,
    DEFAULT_STATION_COORD_JSON_PATH,
    STATION_COORD_JSON_PARAM,
    PlannerConfig,
)
from .planning_plan_a.planner_core import PlannerCore


class PlanningNodePlanA(Node):

    def __init__(self):
        super().__init__('planning_node_plan_a')
        ...
        self.get_logger().info(
            f'[PLAN_A] PlanningNode 시작 | task_topic={task_topic} | '
            f'side={self.side} | '
    ...
    def _steps_to_amr_station_ids(self, steps):
        ...
        for step in steps:
            ...
            if old_station != new_station:
                self.get_logger().info(
                    '[PLAN_A][PLANNER] step station AMR용 복원: '
                    f'step={step.step_id}, '
    ...
    def task_callback(self, task):
        ...
        self.get_logger().info('[PLAN_A] Task 수신 → 계획 생성 시작')
        ...
        try:
            ...
            self.get_logger().info(
                f'[PLAN_A] 계획 생성 완료: {len(self.steps)}개 step'
            )

        except Exception as e:
            self.steps = []
            self.plan_generated = False
            self.get_logger().error(f'[PLAN_A] 계획 생성 실패: {e}')

    def get_plan_callback(self, request, response):
        if not self.plan_generated or not self.steps:
            response.success = False
            response.message = '[PLAN_A] 계획이 아직 생성되지 않았습니다'
            return response
        ...
        self.get_logger().info(
            f'[PLAN_A] GetPlan 응답: {len(self.steps)}개 스텝 전달'
        )
